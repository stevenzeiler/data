...
namespace data::interface {
    ...
    template <typename X>
    concept has_norm_type = has_quad_type<X> && requires (math::quad_type<X> x) {
        { math::sqrt<math::quad_type<X>>{}(x) } -> std::same_as<math::norm_type<X>>;
    } && ordered<math::norm_type<X>>;
}
...
namespace data::interface {
    ...
    template <typename X>
    concept has_sqrt = requires(const X x) {
        { math::sqrt<X>{}(x) };
    };
...
namespace data {
    ...
    template <interface::has_sqrt X>
    inline auto sqrt(const X& x) -> decltype(math::sqrt<X>{}(x)) {
...
namespace data::math {
    ...
    template <interface::has_norm_type X> requires interface::has_quadrance<X> 
    struct norm<X> {
        norm_type<X> operator()(const X& x) const {
            return data::sqrt(data::quadrance(x));
        }
    };
